...
class AttentionBlock(nn.Module):
    def __init__(self, gate_channels, skip_channels, inter_channels):
        ...
        self.W_g = nn.Sequential(
            nn.Conv3d(gate_channels, inter_channels, kernel_size=3, stride = 1, padding = 1),
            nn.BatchNorm3d(inter_channels)
        )
        
        self.W_x = nn.Sequential(
            nn.Conv3d(skip_channels, inter_channels, kernel_size=3, stride = 1, padding = 1),
            nn.BatchNorm3d(inter_channels)
        )
        
        self.psi = nn.Sequential(
            nn.Conv3d(inter_channels, 1, kernel_size=3, stride = 1, padding = 1),
            nn.BatchNorm3d(1),
            nn.Sigmoid()
        )
